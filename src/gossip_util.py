...
def sync_bad_packet(buf, sock, reason="Unknown or malformed data received"):
    print(f"[Gossip] {reason}:")
    hexdump.hexdump(buf)
    print("[Gossip] Exiting.")
    sock.close()
    sys.exit(-1)
...
async def bad_packet(reader, writer, reason='', data=b'', cleanup_func=None):
    try:
        ...
        print(f"[Gossip] (Unknown) xxx Connection closed abruptly, state may remain")
        return

    if reason != '':
        print(f"[Gossip] {raddr}:{rport} >>> {reason}:\n")
    ...
    writer.close()
    await writer.wait_closed()
    print(f"[Gossip] {raddr}:{rport} xxx Connection closed")
...
async def handle_client(reader, writer, handle_message, cleanup_func=None):
    raddr, rport = writer.get_extra_info('socket').getpeername()
    print(f"[Gossip] {raddr}:{rport} >>> Incoming connection")
